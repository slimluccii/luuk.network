...
import { staticHeadersFor, withHeaders } from "./headers.ts";
...
function pathnameOf(request: Request): string {
  return app.removeBase(new URL(request.url).pathname);
}
...
async function standaloneHandler(request: Request): Promise<Response> {
  ...
  if (fileResponse) {
    return withHeaders(fileResponse, new Headers(staticHeadersFor(pathname)));
  }
...
if (config.mode === "middleware") {
  ...
  pullZone
    .onOriginRequest(({ request }) => {
      ...
      if (isSessionPath(pathnameOf(request))) {
    ...
    .onOriginResponse(async ({ request, response }) => {
      ...
      if (response.ok) {
        const pathname = pathnameOf(request);
        const extra = new Headers(staticHeadersFor(pathname));
